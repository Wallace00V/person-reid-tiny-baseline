...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if 'triplet' in cfg.MODEL.LOSS_TYPE:
        triplet = TripletLoss(cfg.SOLVER.MARGIN, cfg.SOLVER.HARD_FACTOR)  # triplet loss

    center_criterion = CenterLoss(num_classes=num_classes, feat_dim=feat_dim, use_gpu=True)  # center loss
    if 'softmax' in cfg.MODEL.LOSS_TYPE:
        if cfg.MODEL.LOSS_LABELSMOOTH == 'on':
            xent = CrossEntropyLabelSmooth(num_classes=num_classes)  # new add by luo
            print("label smooth on, numclasses:", num_classes)

    def loss_func(score, feat, target):
        if cfg.MODEL.LOSS_TYPE == 'triplet+softmax+center':
            ...
            if cfg.MODEL.LOSS_LABELSMOOTH == 'on':
                ...
                return cfg.SOLVER.CE_LOSS_WEIGHT * F.cross_entropy(score, target) + \
                       cfg.SOLVER.TRIPLET_LOSS_WEIGHT * triplet(feat, target)[0] + \
                       cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat, target)
        elif cfg.MODEL.LOSS_TYPE == 'softmax+center':
            #print('Train with center loss, the loss type is triplet+center_loss')
            if cfg.MODEL.LOSS_LABELSMOOTH == 'on':
                return cfg.SOLVER.CE_LOSS_WEIGHT * xent(score, target) + \
                       cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat, target)
                ...
                return cfg.SOLVER.CE_LOSS_WEIGHT * F.cross_entropy(score, target) + \
                       cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat, target)
        elif cfg.MODEL.LOSS_TYPE == 'triplet+softmax':
            #print('Train with center loss, the loss type is triplet+center_loss')
            if cfg.MODEL.LOSS_LABELSMOOTH == 'on':
                return cfg.SOLVER.CE_LOSS_WEIGHT * xent(score, target) + \
                       cfg.SOLVER.TRIPLET_LOSS_WEIGHT * triplet(feat, target)[0]
